...
namespace BLL.Tests
{
    public class OrderServicesTests
    {
        ...
        public void GetInt_Valid()
        {
            ...
            {
                string userLogin = "Customer1";
                int productId = 1;

                mock.Mock<IOrderRepository>()
                    .Setup(x => x.CreateOrder(userLogin, productId));

                var service = mock.Create<OrderServices>();
                service.PlaceOrder(userLogin, productId);

                mock.Mock<IOrderRepository>().
                    Verify(x => x.CreateOrder(userLogin, productId), Times.Exactly(1));
            }
        }
        ...
        public void GetOrderHistory_Valid()
        {
            using (var mock = AutoMock.GetLoose())
            {
                ...
                mock.Mock<IOrderRepository>()
                    .Setup(x => x.GetOrderHistory(userLogin))
                    .Returns(GetSampleOrders());

                var service = mock.Create<OrderServices>();
                var expected = GetSampleOrders();
                var actual = service.GetOrderHistory(userLogin);
                ...
                for (int i = 0; i < expected.Count; i++)
                {
                    ...
                    Assert.Equal(expected[i].UserLogin, actual[i].UserLogin);
        ...
        public void UserOrderCheck_Valid()
        {
            using (var mock = AutoMock.GetLoose())
            {
                ...
                var service = mock.Create<OrderServices>();
                Assert.True(service.UserOrderCheck(userLogin, orderId));
            }
        }
